...
#+RESULTS:
: ((font . JetBrains Mono 14) (alpha-background . 80) (vertical-scroll-bars) (left-fringe . 10) (right-fringe . 10))
...
#+begin_quote
BEST PRACTICE:
If the function has a forward declaration (especially one in a header file), put the default argument there. Otherwise, put the default argument in the function definition. For example, in the forward declaration do this ~int add(int x=5, int y=7)~ and in the definition, do this ~int add(int x, int y)~.
#+end_quote
* Function Templates
...
#+RESULTS:
- When class types need to be used in multiple files, the following can be done-
  1. The class definition with its member variables and /forward declarations/ of member functions is defined in a header file with the same name as the class.
  2. All the member functions are defined in a cpp file having the same name as that of the header file.
  3. Trivial member functions like getters can be defined inside the class definition.
- You can also make the member functions ~inline~ and define them in the single header file, however, this is not a recommended practice.
** Nested Types
...
** Constructing arrays of specific length
...
Checkout clockwise/spiral rule or the right/left rule for understanding complex C/C++ declarations.
...
| Parin  |
| Parin  |
** Defining Variables in Capture Block
...
- Look at this example for details: [[https://www.learncpp.com/cpp-tutorial/shallow-vs-deep-copying/][learncpp.com]]
*** Deep Copy
...
#+begin_src cpp :wrap example :results raw
#include <iostream>

...
- We can use the ~.at()~ member function to access elements with runtime bounds checking.
...
- ~std::string~ and ~std::vector~, both support move semantics, so they CAN be safely passed by VALUE!!!
